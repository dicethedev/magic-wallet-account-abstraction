...
const RECIPIENT_ADDRESS = "0x00000";
...
export default function Dashboard() {
  ...
  const [numberToSet, setNumberToSet] = useState("");
  ...
  const mint = async () => {
    ...
    try {
      const provider = new ethers.BrowserProvider(magic.rpcProvider);
      const signer = await provider.getSigner();
      const contractAddress = "0x66a25d863FEF11f0fa069E9398DE09B7B63Eb296";
      ...
      // Populate the transaction with the fixed recipient address
      const transaction = await contract.mint.populateTransaction(
        amountInWei,
        RECIPIENT_ADDRESS
      );
  ...
  const setNumber = async () => {
    ...
    try {
      const provider = new ethers.BrowserProvider(magic.rpcProvider);
      const signer = await provider.getSigner();
      const contractAddress = "0x66a25d863FEF11f0fa069E9398DE09B7B63Eb296"; // Replace with your actual contract address
      ...
      const num = numberToSet;
      // Populate the transaction for setNumber
      const transaction = await contract.setNumber.populateTransaction(num);

      // Send the gasless transaction
      const result = await magic.wallet.sendGaslessTransaction(
        user.publicAddress, // Replace with the correct address if necessary
        transaction
      );

      console.log("setNumber Transaction hash:", result);
      setTransactionHash(result);

      // Wait for the transaction to be mined
      const receipt = await provider.waitForTransaction(result);
      console.log("setNumber Transaction receipt:", receipt);

      // Optionally refresh state after setting the number
      // getBalance(); // Uncomment if needed
    } catch (error) {
      console.error("An error occurred during setNumber:", error);
      setError("An error occurred during setNumber: " + error.message);
    } finally {
      setLoadingNum(false);
    }
  };
        ...
        <>
          ...
          <center>
            <input
              ...
              value={numberToSet}
              onChange={(e) => setNumberToSet(e.target.value)} // Update state on input change
              style={{ marginBottom: "10px" }}
            />
            <button
              onClick={setNumber} // Call setNumber on button click
              style={{ display: "block" }}
              disabled={loadingNum} // Disable button while loading
            >
              {loadingNum ? "Setting Number..." : "Set Number"}
            </button>
            {error && (
              <p style={{ color: "red" }}>
                {typeof error === "string" ? error : JSON.stringify(error)}
